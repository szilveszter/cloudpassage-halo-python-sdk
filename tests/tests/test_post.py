...
post = imp.load_module('post', file, filename, data)
...
class TestGet:
    def test_post_404(self):
        ...
        req = post.Post(session)
        try:
            json_response = req.post(url, post_data)
        except post.CloudPassageResourceExistence:
            pathfailed = True
        assert pathfailed
    ...
    def test_post_bad_payload(self):
        ...
        try:
            json_response = req.post(url, post_data)
        except post.CloudPassageValidation:
            rejected = True
        assert rejected
